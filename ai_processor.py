...
- "记忆要点"章节要根据笔记内容和老师讲课重点，提炼出便于记忆的关键信息，使用简洁的表述和恰当的emoji符号
- "相关概念"章节只需要有标题即可，内容可以为空，便于后续手动或自动补充
- 其他章节完全根据内容需要自由创造
...
(不需要填写，由用户自行填写，只需要有上面的标题“## 相关概念”即可)
...
请开始分析字幕内容。记住：你正在为法考知识图谱创建节点，为法考Wiki创建条目，为错题整理系统创建知识点索引。每个笔记都要达到这三个目标的要求。倾向于超细化拆分，为每个独立概念创建完整而详细的笔记："""
