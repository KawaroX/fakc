...
class SiliconFlowConceptEnhancer:
    ...
    def _build_hybrid_enhanced_prompt(
        ...
        return f"""你是法考知识体系专家。请分析以下笔记，基于BGE模型混合检索筛选出的高度相关概念，检查并优化其概念关系链接。
...
BGE混合检索的相关概念（embedding召回+reranker精排）：
{concepts_list}

要求：
1. 这些概念都是经过BGE嵌入模型和重排序模型双重筛选的高质量候选
2. 仔细分析笔记内容与每个概念的实际关联性
3. 优先考虑相关度分数高的概念，但最终以语义相关性为准
4. 确保新增的概念链接与笔记内容高度相关，严格把控添加的概念是否真的是相关的
5. 主要关注同类型概念、上下级概念、相似或者易混淆和笔记内容里出现的这四类型概念。不要太随意添加相关概念
6. 使用双链显示别名格式：[[【科目】概念名|概念名/别名]]。请务必采用这种格式，特别确保【科目】部分正确填写。
7. 移除指向不存在或确实不相关的概念链接
8. 不要添加任何优化说明或额外内容
